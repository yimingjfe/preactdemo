...
(function () {
	...
	function idiff(dom, vnode, context, mountAll, componentRoot) {
			...
			// 如果不存在dom对象或者节点的tag变了
			if (!dom || !isNamedNode(dom, vnodeName)) {
					out = createNode(vnodeName, isSvgMode);
					// 如果只是类型变了，会把原dom的子节点重新挂载到新的dom上，便于diffChildren的比较
					if (dom) {
			...
			// 把dom节点的attributes都放在了dom['__preactattr_']上，这点有什么用？
			if (props == null) {
					props = out['__preactattr_'] = {};
			...
			// 如果dom和vchildren都只有一个节点，且是textnode节点时，直接改老dom的nodeValue
	...
	function innerDiffNode(dom, vchildren, context, mountAll, isHydrating) {
			...
			// 遍历原dom本来的数组，有key的child放在keyed里面，没有key的children放入children数组
			if (len !== 0) {
					for (var i = 0; i < len; i++) {
					...
					for (var i = 0; i < vlen; i++) {
							...
							if (key != null) {
									if (keyedLen && keyed[key] !== undefined) {
											child = keyed[key];                         // 根据key找到的原dom子节点
											keyed[key] = undefined;
											keyedLen--;
									}
							}
	...
	function collectComponent(component) {
			var name = component.constructor.name;  // 这个时候props和state还在，所以重新利用的时候是怎么回事？
			...
			if (list) {
					for (var i = list.length; i--;) {
							if (list[i].constructor === Ctor) {
									debugger    // 想看一下此时dom上preact添加的属性是否已经清空？比如dom._component dom['__preact_']
	...
	function renderComponent(component, opts, mountAll, isChild) {
			...
			var props = component.props,
					...
					isUpdate = component.base,                  // 当前组件dom已经被渲染，非第一次渲染
	...
	function unmountComponent(component) {
			...
			var base = component.base;  //component.base是离组件最近的dom节点
			...
			var inner = component._component;   // 指向子组件的实例， 只可能有一个根组件
			if (inner) {            // 如果孩子是子组件的话，只要向下递归的卸载子组件就好了
					...
					removeNode(base);   // 从dom上删除对应的节点
					collectComponent(component);  // 将组件放入components[component.constructor.name]里面
