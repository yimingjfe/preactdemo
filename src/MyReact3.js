...
function importFromBelow(){
  ...
  function diff2(instance, elem, parentDom) {
    if (instance == null) {
      // Create instance
      const newInstance = instantiate(elem);
      ...
      return null;
    } else if (instance.elem.type !== elem.type) {
      // Replace instance
      const newInstance = instantiate(elem);
      parentDom.replaceChild(newInstance.dom, instance.dom);
      return newInstance;
    } else if (typeof elem.type === "string") {
      // Update instance
      updateDomProperties(instance.dom, instance.elem.props, elem.props);
      instance.childInstances = reconcileChildren(instance, elem);
      instance.elem = elem;
      return instance;
    } 
      //Update composite instance
      instance.publicInstance.props = elem.props;
      const childElement = instance.publicInstance.render(); // 用新的属性重新渲染
      const oldChildInstance = instance.childInstance;
      const childInstance = diff(  // 老的实例与新的实例比较，便于更改dom
        parentDom,
        oldChildInstance,
        childElement
      );
  ...
  function instantiate(elem){
    if(typeof elem.type === 'string'){  // 普通的元素
      ...
      let dom = isTextNode ? document.createTextNode(elem.props.nodeValue)   //那个项目是怎么渲染的？
